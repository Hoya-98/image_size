...
sam_checkpoint = "../prostate_gradio/checkpoint/sam_vit_b_01ec64.pth"
model_type = "vit_b"
...
def mask_generate(img, text, sam):
    ...
    for mask, score in zip(masks, scores):
        ...
        mask_with_image = cv2.addWeighted(img.astype(np.uint8), 0.75, (mask_image*255).astype(np.uint8), 0.5, 0)
...
def preprocess(output):
    ...
    mask_with_image1, mask_with_image2, mask_with_image3, mask_image1, mask_image2, mask_image3 = mask_generate(output['image'], [x1, y1, x2, y2], sam)
...
if __name__=="__main__":
    ...
    demo.launch(share=True, server_name="0.0.0.0", server_port=8050)
#######################################################################################################################################